...
// netlify/functions/publishPortfolio.mjs
var publishPortfolio_exports = {};
__export(publishPortfolio_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(publishPortfolio_exports);
...
// netlify/functions/blobStore.mjs
...
function buildPublishUrl(event, slug) {
  ...
  const proto = isLocal ? "http" : event.headers["x-forwarded-proto"] || event.headers["X-Forwarded-Proto"] || "https";
...
async function handler(event) {
  ...
  const { data: membership, error: membershipError } = await supabase.from("memberships").select("downloads_used, downloads_limit, status").eq("user_id", user.id).single();
  ...
  try {
    ...
    const { error: updateError } = await supabase.from("memberships").update({ downloads_used: deployNumber }).eq("user_id", user.id);
    if (updateError) {
...
//# sourceMappingURL=publishPortfolio.js.map
