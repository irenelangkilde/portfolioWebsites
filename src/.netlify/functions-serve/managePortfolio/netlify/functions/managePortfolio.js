...
// netlify/functions/managePortfolio.mjs
var managePortfolio_exports = {};
__export(managePortfolio_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(managePortfolio_exports);
...
async function handler(event) {
  ...
  const { data: membership } = await supabase.from("memberships").select("downloads_used, downloads_limit, status").eq("user_id", user.id).single();
  ...
  if (action === "delete") {
    ...
    try {
      ...
      return json(500, { error: `Delete failed: ${explainBlobStoreError(err)}` });
    }
...
//# sourceMappingURL=managePortfolio.js.map
