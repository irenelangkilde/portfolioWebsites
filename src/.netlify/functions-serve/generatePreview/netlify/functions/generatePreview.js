...
// netlify/functions/generatePreview.mjs
...
function loadPromptTemplate() {
  ...
  const p1 = (0, import_path2.resolve)(cwd, "src/Prompt for Portfolio Website Generation-Claude.txt");
  try {
    return (0, import_fs2.readFileSync)(p1, "utf-8");
  } catch {
  }
  const p2 = (0, import_path2.resolve)(cwd, "FirstOutputPrompt.txt");
  try {
    return (0, import_fs2.readFileSync)(p2, "utf-8");
  } catch {
  }
  throw new Error(`Cannot find FirstOutputPrompt.txt. Tried: "${p1}" and "${p2}" (cwd: "${cwd}")`);
}
function fillTemplate(template, vars) {
  return template.replace(/\{\{([A-Z_]+)\}\}/g, (_2, key) => vars[key] ?? "");
}
...
async function handler(event) {
  try {
    ...
    const client = new openai_default({ apiKey: process.env.OPENAI_API_KEY });
