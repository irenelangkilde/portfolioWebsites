...
// netlify/functions/getGiftCode.mjs
var getGiftCode_exports = {};
__export(getGiftCode_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(getGiftCode_exports);
...
async function handler(event) {
  ...
  const { data, error } = await supabase.from("gift_codes").select("code, tier").eq("stripe_session_id", sessionId).single();
  if (error || !data) {
...
//# sourceMappingURL=getGiftCode.js.map
