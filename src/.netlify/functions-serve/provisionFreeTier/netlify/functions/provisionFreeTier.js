...
// netlify/functions/provisionFreeTier.mjs
var provisionFreeTier_exports = {};
__export(provisionFreeTier_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(provisionFreeTier_exports);
...
function loadLocalEnv() {
  ...
  localEnvCache = {};
  const candidates = [(0, import_path.resolve)(process.cwd(), ".env"), (0, import_path.resolve)(process.cwd(), "../.env")];
...
async function handler(event) {
  ...
  const { data: existing } = await supabase.from("memberships").select("tier").eq("user_id", userId).maybeSingle();
...
//# sourceMappingURL=provisionFreeTier.js.map
