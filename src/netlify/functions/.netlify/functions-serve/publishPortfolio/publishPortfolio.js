...
async function handler(event) {
  ...
  const { data: membership, error: membershipError } = await supabase.from("memberships").select("deploys_used, deploys_limit, status").eq("user_id", user.id).single();
  ...
  try {
    ...
    const { error: updateError } = await supabase.from("memberships").update({ deploys_used: deployNumber }).eq("user_id", user.id);
    if (updateError) {
